...
- Flow: proteos_machines_list to get a machine id → proteos_project_ensure the repo onto it → proteos_task_run with a clear prompt. If no suitable machine exists, create one from a template (proteos_templates_list → proteos_machine_create); if a machine is stopped, proteos_machine_start it. task_run returns a task id immediately and does NOT wait — by default, just report the id and end your turn; do not loop on proteos_task_get to wait for it. Only pass wait:true to task_run (or poll manually) when the user explicitly asks to wait for or be notified about the result. Every proteos call takes the machine id explicitly; task/git/project calls also take the project (the repo's workspace directory name). If a machine is running a task for the same project, do proteos_task_run in a different machine.
